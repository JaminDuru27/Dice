...
export function Register({setAuthState, onSubmit = ()=>{}}){
    ...
    return (
        ...
        className=" w-full  h-fit z-10 overflow-hidden border border-white/30 bg-black/90 rounded-2xl p-4  ">
            ...
            <div className="auth state w-full text-center my-4">Register</div>
            <div>
                <Input title="name" key={0} required={true} onInput={(v)=>{name[`current`] = v}}/>
                <Input title="unique name or email" key={0} required={true} onInput={(v)=>{uniquename[`current`] = v}}/>
                <Input title="password" type="password" key={0} required={true} onInput={(v)=>{password[`current`] = v}}/>
            </div>
            <div className="text-[.8rem] my-4 ">or<span 
            onClick={()=>{
                setAuthState(`login`)
            }}
            className="border-b-white cursor-pointer p-2 w-fit">Login</span></div>
            <div 
            onClick={async ()=>{onSubmit({state:`register`, userUniqueId:uniquename.current, username:name.current, password:password.current})}}
