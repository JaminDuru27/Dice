...
export function Log({text, by, key, time, onSeen=()=>{}, status}){
    ...
    return (
        ...
        className={`flex relative items-center w-screen  h-fit gap-2 `}>
            <div className="circle absolute top-1/2 left-0 translate-x-[-200%] translate-y-[-50%] w-2 h-2 rounded-full bg-green-600/90 "></div>
            <div className="icon"> 
            ...
            className="relative p-1 px-1 sm:p-2 sm:px-4 rounded-2xl bg-white/20 w-fit  overflow-hidden border border-white/20 text-[.8rem] ">
                ...
                className="time text-end text-[.6rem] text-white/60 capitalize text-[50%] sm:text-[80%] md:text-[100%] flex items-center gap-2">{<Send size={10}/>}{status}</motion.div>
